...
maxxlimit=50000 # Use fixed value not auto, so that plots made anytime during training have same scale
maxticks=10
...
if not e.dropna().empty: # not NaN or empty
	...
	ax1.scatter(x, e, c='magenta', s=50, label='Evaluation CER from lstmtraining')
	annot_min('magenta',-0,40,x,e) 

if not v.dropna().empty: # not NaN or empty
	ax1.plot(x, v, 'red')
	ax1.scatter(x, v, c='red', s=50, label='Validation CER from lstmeval')
	annot_min('red',-0,40,x,v)
